...
// =============================================================================
//  BIT LIST -- up to 20 slots below (Bit defaults, then Bit list). Turn a
//  slot off with its "enabled" checkbox to skip it entirely; disabled
//  slots don't affect sizing/layout at all. All dimensions in mm, measured
//  from the head end downwards: head = cutting head (widest), neck =
//  shaft, base = shank that goes in the chuck. Each bit's engraved label
//  is generated from its head_dia automatically (how forstner bits are
//  normally marked).
//
...
//  (head_len, neck_dia, neck_len, base_dia, base_len) defaults to 0,
//  meaning "use the matching default_* value in Bit defaults" -- most
//  forstner sets share the same head length and shank across many sizes,
//  so this saves re-typing the same five numbers 20 times. Give a bit its
//  own non-zero value in any of those fields to override the default just
//  for that bit (e.g. if the shaft is a single constant diameter, set
//  that bit's base_dia equal to its neck_dia).
//
...
// base_dia/base_len at 0. Does not apply to head_dia -- that always comes
...
// sort bit INDICES by head diameter, largest first, so all six lists stay
// in sync instead of sorting (and duplicating) each list separately
...
function bdia(i) = base_dias[order[i]];
function blen(i) = base_lens[order[i]];
function lbl(i) = str(hd(i));
function bitlen(i) = hl(i) + nl(i) + blen(i);

// diameter of bit i at distance d from its head end
function dia_at(i, d) = d < hl(i)         ? hd(i) :
                        d < hl(i) + nl(i) ? nd(i) : bdia(i);
...
function wL(i) = flip(i) ? bdia(i) : hd(i);
function wR(i) = flip(i) ? hd(i) : bdia(i);
